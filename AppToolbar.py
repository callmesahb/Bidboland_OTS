...
# """)
        ...
        self.MessageChecker = QtCore.QTimer()
        self.MessageChecker.timeout.connect(self.CheckTIMER)
        self.MessageChecker.start(2000)
        
        
        
    def CheckTIMER(self):
        self.last_message = self.aspen.get_last_message()
        self.last_message2 = self.aspen.get_last_message2()
        self.last_message3 = self.aspen.get_last_message3()
        timer1 = self.aspen.GetSimTimer()
        timer2 = self.aspen.GetSimTimer2()
        timer3 = self.aspen.GetSimTimer3()
        
        if "terminated" in self.last_message and not self.show_terminate and timer1 > 0.0:
            ...
            self.show_terminate = True
        ...
        elif "terminated" in self.last_message2 and not self.show_terminate and timer2 > 0.0:
                message = QtWidgets.QMessageBox()
                message.setText("The Unit is Shut Down.\nPlease Perform a Rewind Operation.")
                message.setWindowTitle("ERROR")
                message.setWindowFlags(QtCore.Qt.WindowType.WindowStaysOnTopHint)
                message.exec()
                self.show_terminate = True
        elif "terminated" in self.last_message3 and not self.show_terminate and timer3 > 0.0:
                message = QtWidgets.QMessageBox()
                message.setText("The Unit is Shut Down.\nPlease Perform a Rewind Operation.")
                message.setWindowTitle("ERROR")
                message.setWindowFlags(QtCore.Qt.WindowType.WindowStaysOnTopHint)
    ...
    def RunAPI(self):
        exe_path = r"C:\\BB_OTS5\\API\\Run_Sim.exe"
    ...
    def RewindSim(self):
        exe_path = r"C:\\BB_OTS5\\API\\Rewind_Sim.exe"
        ...
        self.store.settingValueOPC("403ARSTIMER", 0)
        print(self.store.history)
    ...
    def readingdata(self):
        timer_value = int(self.store.finaltag["403ARSTIMER"])
