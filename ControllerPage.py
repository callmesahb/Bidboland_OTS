...
class ControllerPage(QtWidgets.QWidget):
    notchangingsp = QtCore.pyqtSignal(bool)
    ...
    def init_ui(self):
        ...
        self.tabs.addTab(MainTab(self.store,self.variableid,self.name,self.pvvalues,self.ranges,self.itype,self.title),"Main")
        self.tabs.addTab(LoopTune(self.itype,self.variableid,self.store,self.ranges),"Loop Tune")
        self.spval = SetPoint(self.variableid,self.store,self.ranges)
        self.spval.notchangingsp.connect(self.change)
        self.tabs.addTab(self.spval,"Set Point")
        ...
        self.tabs.addTab(Chart(self.itype,self.variableid,self.store,self.ranges),"Chart")
    ...
    def change(self,status:bool):
        if status == True:
            self.contplate.SPValue.setEnabled(False)
        else:
            self.contplate.SPValue.setEnabled(True)
class MainTab(QtWidgets.QWidget):
    def __init__(self,store:Store,variableid,name,pvvalues,ranges,itype,title):
        super().__init__()
        self.fontt = QtGui.QFont("New Times Roman")
        self.store = store
        self.variableid = variableid
        self.contplate = ControllerPlate(name,pvvalues,variableid,store,ranges,itype,title)
        self.contplate.Updatestatus.connect(self.Updatestatus)
    ...
    def Updatestatus(self,status:str):
        if status == "LL ALARM":
            ...
            self.highradio.setChecked(False)
            self.moduleradio.setChecked(True)
            self.moduleradio.setText("ON")
            self.lowradio.setText("ON")
            self.highradio.setText("OFF")
        elif status == "HH ALARM":
            self.highradio.setChecked(True)
            self.lowradio.setChecked(False)
            self.moduleradio.setChecked(True)
            self.moduleradio.setText("ON")
            self.highradio.setText("ON")
            self.lowradio.setText("OFF")
        elif status == "NORMAL":
            self.highradio.setChecked(False)
            self.lowradio.setChecked(False)
    ...
    def alarmenableandsummary(self):
        ...
        self.highradio = QtWidgets.QRadioButton("OFF")
        self.highradio.setEnabled(False)
        self.highradio.setStyleSheet("""
        ...
        self.moduleradio.setEnabled(False)
        ...
        self.lowradio = QtWidgets.QRadioButton("OFF")
        self.lowradio.setEnabled(False)
        self.lowradio.setStyleSheet("""
                QRadioButton::indicator {
                    background-color: red;
                    border: 1px solid gray;
                    width: 15px;
                ...
                """)
        g5layout.addWidget(self.highradio,5,1)
        g5layout.addWidget(self.lowradio,6,1)
        for row in [4, 7, 8, 9, 10]:
            ofcheck = QtWidgets.QRadioButton("OFF")
            ofcheck.setEnabled(False)
            ofcheck.setStyleSheet("""
                QRadioButton::indicator {
                    background-color: red;
                    border: 1px solid gray;
                    width: 15px;
        ...
        self.group.addButton(self.highradio)
        self.group.addButton(self.lowradio)
        self.group.addButton(self.moduleradio)
...
class SetPoint(QtWidgets.QWidget):
    notchangingsp = QtCore.pyqtSignal(bool)
    def __init__(self,variableid,store,ranges):
        super().__init__()
        self.fontt = QtGui.QFont("New Times Roman")
        self.fontt.setBold(True)
    ...
    def SPtracking(self):
        ...
        self.spvalue.returnPressed.connect(self.changesp)
        self.spvalue.setReadOnly(False)
        self.spvalue.setEnabled(False)
        self.sptext = self.spvalue.text()
        ...
        enablepvtracking.setChecked(False)
        ...
        self.SPRamping()
    def changesp(self):
        new_sp = float(self.spvalue.text())
        varid = self.variable + "SP"
        if new_sp != self.sptext:
            self.sptext = new_sp
            self.store.settingValueOPC(varid,new_sp)
    def putspadinopc(self):
        ...
        if self.enablingspprocessing.isChecked():
            ...
            self.notchangingsp.emit(True)
        else:
            self.spValue = self.spvalue.text()
            self.notchangingsp.emit(False)
...
class Alarms(QtWidgets.QWidget):
    emitingvalueofhigh = QtCore.pyqtSignal(float,float)
    def __init__(self,variableid,store:Store,name,pvvalues,ranges,type,title):
        ...
        # self.emitingvalueofhigh.connect(self.contplate.ChangingValueofhighrange)
        self._initUI()
    ...
    def checkingvalue(self):
        ...
        self.pvhighvalue.setPlaceholderText(str(pvhv))
        self.pvlowvalue.setPlaceholderText(str(pvlv))
    def _initUI(self):
    ...
    def Alarmconf(self):
        ...
        g1layout.addWidget(pvhighhighvalue,2,4)
        self.pvhighvalue = QtWidgets.QLineEdit()
        # self.pvhighvalue.setText(str(highvalue))
        self.pvhighvalue.setEnabled(True)
        self.pvhighvalue.setFixedSize(80,25)
        self.pvhighvalue.setText(str(self.pvhv))
        g1layout.addWidget(self.pvhighvalue,3,4)
        self.pvhighvalue.returnPressed.connect(self.emitingvalues)
        self.pvlowvalue = QtWidgets.QLineEdit()
        self.pvlowvalue.setText(str(self.pvlv))
        # self.pvlowvalue.setText(str(lowvalue))
        self.pvlowvalue.setEnabled(True)
        self.pvlowvalue.returnPressed.connect(self.emitingvalues)
        self.pvlowvalue.setFixedSize(80,25)
        g1layout.addWidget(self.pvlowvalue,4,4)
        self.status = "OFF"
    ...
    def emitingvalues(self):
        valuehightext = self.pvhighvalue.text()
        valuelowtext = self.pvlowvalue.text()
        varhid = self.variableid + "PVH"
        varlid = self.variableid + "PVL"
        pvhv = self.store.finaltag[varhid]
        pvlv = self.store.finaltag[varlid]
        if float(valuehightext) > self.ranges[1]:
            valuehightext = self.ranges[1]
        elif float(valuelowtext) < self.ranges[0]:
            valuelowtext = self.ranges[0]
        self.pvlowvalue.setText(str(valuelowtext))
        self.pvhighvalue.setText(str(valuehightext))
        self.store.settingValueOPC(varhid,float(valuehightext))
        self.store.settingValueOPC(varlid,float(valuelowtext))
        # self.emitingvalueofhigh.emit(float(valuehightext),float(valuelowtext))
    @QtCore.pyqtSlot(str)
...
class LoopTune(QtWidgets.QWidget):
    def __init__(self,itype,variableid,store,ranges):
        ...
        self.action = self.store.gainvalueextraction(variableid)
        self._InitUI()
    def _InitUI(self):
        self.vlayout1 = QtWidgets.QVBoxLayout()
        self.hlayout = QtWidgets.QHBoxLayout()
        self.runchart()
    def runchart(self):
        self.trend = Trend(self.variableid,"chtrend",self.store,self.ranges)
