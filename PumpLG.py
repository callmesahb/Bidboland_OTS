...
class NormalPumpLG(QWidget):
        ...
        def ReadingValue(self):
            ...
            v1 = self.store.finaltag["420P103ALG"]
            v2 = self.store.finaltag["420P103BLG"]
            # print(f"{varid}:{value}:{self.fvalue}")
            if self.variableid == "420P103A" or self.variableid == "420P103B":
                    orgvarid = self.variableid + "LGORG"
                    orgvaridvalue = self.store.finaltag[orgvarid]
                    var1 = self.store.finaltag["420P103ALGORG"]
                    var2 = self.store.finaltag["420P103BLGORG"]
                    if v1 == 2:
                        self.store.settingValueOPC("420P103A",v1)
                    self.store.settingValueOPC("420P103B",v2)
                    if orgvaridvalue == 1:
                        self.setWindowOpacity(0.0)
                    elif orgvaridvalue == 2:
                        self.setStyleSheet("""
                            QWidget {
                                background: #676767;
                    ...
                    elif var1 == 2.0 and var2 == 2.0:
                        self.store.settingValueOPC("420P103A",2)
                        self.store.settingValueOPC("420P103B",2)
            else:
