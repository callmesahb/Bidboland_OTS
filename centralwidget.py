...
from ComprossorElem import Comprossor
...
class MainWidget(QtWidgets.QWidget):
    ...
    def __init__(self, store: Store,boundarytags):
        super().__init__()
        ...
        self.store.updatevalues.connect(self.ChangeBoundaryValue)

    def ChangeBoundaryValue(self):
        for connection in self.boundarytags:
            for source, target in connection.items():
                value = self.store.finaltag[source]
                if value is not None:
                    self.store.settingValueOPC(target, value)
        ...
        for page in pages:
            ...
            for _valve in page["valves"]:
                ...
                if _type == "controller" and variableid in self.store.getting_names():
                    ...
                    valve =DosingPump(self.store,variableid, value,rotated,title)
                    self.store.updatevalues.connect(valve.ReadingValue)
                    # valve.set_status(value)
                elif _type == "fan":
                    ...
                    valve = Comprossor(self.store,variableid,name,value,title)
                    self.store.updatevalues.connect(valve.ReadingValue)
